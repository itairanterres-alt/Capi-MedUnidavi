// Capi Chat (contextual pedagogical AI panel). Ported from the prototype's
// med-unidavi-capi-chat.jsx. Renders only on enabled screens (questoes, ic,
// reflexivo, simdavi, morfofuncional) and in "orientador" mode on
// dashboard/ppc/trilhas. Separate from the human Canal FAB.
import React from 'react';
import { DS } from '../lib/ds.js';
import { CapiSprite, CAPI_POSE_BY_SCREEN } from './Capi.jsx';
...
export function CapiChat({ telaAtual }) {
  ...
  return (
    <React.Fragment>
      <button onClick={() => setAberto(true)} aria-label="Capi · apoio pedagógico" title={`Capi — apoio pedagógico desta tela (${ctx.titulo})`}
        style={{ position: 'fixed', right: 18, bottom: 80, zIndex: 89, width: 64, borderRadius: 14, background: DS.surface, border: `1.5px solid ${DS.border}`, cursor: 'pointer', boxShadow: '0 6px 20px rgba(8,16,32,0.20)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 0, overflow: 'hidden', transition: 'box-shadow 0.15s, transform 0.15s' }}
        ...
        <div style={{ width: '100%', padding: '4px 0', background: isOrientador ? DS.terraLight : DS.blueLight, borderTop: `1px solid ${DS.border}`, fontSize: 9.5, fontWeight: 800, color: isOrientador ? DS.terra : DS.blue, textAlign: 'center', letterSpacing: '0.5px' }}>{isOrientador ? 'PERCURSO' : 'CAPI'}</div>
      </button>

      {aberto && (
        <div onClick={() => setAberto(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(8,16,32,0.40)', zIndex: 100, display: 'flex', alignItems: 'flex-end', justifyContent: 'flex-end' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 420, height: '100%', maxHeight: '88vh', background: DS.surface, borderTopLeftRadius: 14, borderBottomLeftRadius: 14, display: 'flex', flexDirection: 'column', boxShadow: '-6px 0 24px rgba(0,0,0,0.18)' }}>
            <div style={{ padding: '14px 16px', borderBottom: `1px solid ${DS.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11, color: DS.textMuted }}>{isOrientador ? 'Orientador · jornada formativa' : 'IA pedagógica · contexto desta tela'}</div>
              </div>
            ...
            <div style={{ padding: '10px 16px', background: isOrientador ? DS.blueLight : DS.amberLight + '50', borderBottom: `1px solid ${DS.border}`, fontSize: 11, color: DS.textSec, lineHeight: 1.5 }}>
                ...
                <div style={{ color: DS.textMuted, fontSize: 12, textAlign: 'center', padding: '16px 8px', lineHeight: 1.6 }}>
                  {isOrientador ? (
                    <div style={{ textAlign: 'left' }}>
                      <div style={{ marginBottom: 12, color: DS.textSec, lineHeight: 1.55 }}>Estou acompanhando seu percurso formativo.</div>
                      {_sinais.map((s, i) => (
                        <div key={i} onClick={() => { setHist(h => [...h, { role: 'user', text: s.cta }, { role: 'capi', text: 'Vou te ajudar a retomar. Que tal reservar 10 minutos agora? O FSRS já selecionou o mais urgente.' }]); }}
                          style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 8, marginBottom: 8, background: DS.bg, border: `1px solid ${DS.border}`, cursor: 'pointer', transition: 'background 0.1s' }}
                    ...
                    <React.Fragment>Inicie uma pergunta sobre o conteúdo desta tela.<br/><span style={{ fontSize: 11 }}>Não fornece diagnóstico, conduta clínica ou redação acadêmica final.</span></React.Fragment>
                  )}
                </div>
              )}
              {hist.map((m, i) => (
                <div key={i} style={{ alignSelf: m.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '85%', padding: '9px 13px', borderRadius: 12, background: m.role === 'user' ? DS.blue : DS.bg, color: m.role === 'user' ? '#fff' : DS.text, fontSize: 13, lineHeight: 1.5, border: m.role === 'capi' ? `1px solid ${DS.border}` : 'none' }}>{m.text}</div>
              ))}
            </div>
            <div style={{ padding: 12, borderTop: `1px solid ${DS.border}`, display: 'flex', gap: 8 }}>
              <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') enviar(); }} placeholder={isOrientador ? 'Como posso apoiar sua jornada…' : 'Pergunte ao Capi sobre esta tela…'}
