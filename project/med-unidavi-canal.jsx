// =============================================================================
// MED-UNIDAVI · Canal de Comunicação Institucional (FAB global)
// FAB bottom-right em todas as telas exceto Login. Modal com 4 caminhos.
// Cada acionamento registra evento institucional (data, tela, demanda).
// =============================================================================

const CANAL_OPCOES = [
  {
    id: 'tutoria',
    titulo: 'Falar com a Tutoria',
    sub: 'Estudante → tutor designado',
    icon: '🎓',
    color: '#1D50A8',
    perfis: ['estudante'],
    detalhe: 'Comunicação direta com seu tutor. Use para dúvidas sobre PBL, encontros tutoriais, EPAs do logbook ou orientação acadêmica.',
  },
  {
    id: 'coordenacao',
    titulo: 'Falar com a Coordenação',
    sub: 'Estudante ou docente → coordenação adjunta',
    icon: '🏛️',
    color: '#0F6A47',
    perfis: ['estudante', 'docente'],
    detalhe: 'Demandas institucionais: matrícula, aproveitamento, calendário, NDE, conflitos. Resposta em até 3 dias úteis.',
  },
  {
    id: 'suporte',
    titulo: 'Suporte técnico da plataforma',
    sub: 'Bugs, acesso, dados',
    icon: '🛠️',
    color: '#C4622D',
    perfis: ['estudante', 'docente', 'coordenacao'],
    detalhe: 'Problemas de login, dados não exibidos, erros de tela, dúvida sobre a plataforma. Time de TI da MED-UNIDAVI.',
  },
  {
    id: 'institucional',
    titulo: 'Atendimento institucional UNIDAVI',
    sub: 'Setores administrativos · link out',
    icon: '🏫',
    color: '#7A5CBF',
    perfis: ['estudante', 'docente', 'coordenacao'],
    detalhe: 'Financeiro, secretaria, RH, ouvidoria, biblioteca central. Abre o portal institucional unidavi.edu.br em nova aba.',
    extern: true,
  },
];

function CanalFAB({ telaAtual, perfil = 'estudante' }) {
  const [aberto, setAberto] = React.useState(false);
  const [confirmado, setConfirmado] = React.useState(null); // { opcao, ts }

  const opcoesPerfil = CANAL_OPCOES.filter(o => o.perfis.includes(perfil));

  const acionar = (opcao) => {
    const evento = {
      tipo: 'canal_comunicacao',
      opcao: opcao.id,
      tela: telaAtual || 'desconhecida',
      perfil,
      ts: new Date().toISOString(),
    };
    // Em produção: post para backend institucional. Aqui, log + confirmação visual.
    if (typeof console !== 'undefined') console.log('[evento institucional]', evento);
    if (opcao.extern) {
      window.open('https://www.unidavi.edu.br', '_blank', 'noopener,noreferrer');
      setAberto(false);
      return;
    }
    setConfirmado({ opcao, ts: evento.ts });
  };
    ...
    <React.Fragment>
      ...
      <button
        ...
        style={{
          ...
          boxShadow: '0 6px 18px rgba(29,80,168,0.35)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 22, fontFamily: 'inherit',
        }}
        title="Canal institucional · fale com tutoria, coordenação, suporte ou setores UNIDAVI"
      >💬</button>
      ...
      {aberto && (
        <div
          onClick={fechar}
          style={{
            position: 'fixed', inset: 0, background: 'rgba(8,16,32,0.55)',
        ...
        >
          <div
            ...
            style={{
              background: DS.surface, borderRadius: 14, maxWidth: 520, width: '100%',
            ...
            <div style={{ padding: '18px 22px 14px', borderBottom: `1px solid ${DS.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: DS.textMuted, marginTop: 3 }}>
                  Tela atual: <strong>{telaAtual || '—'}</strong> · perfil: <strong>{perfil}</strong> · cada acionamento é registrado
                </div>
              </div>
                ...
                {opcoesPerfil.map(o => (
                  <button
                    ...
                    style={{
                      ...
                      padding: 14, borderRadius: 10, background: DS.bg,
            ...
            {confirmado && (
              <div style={{ padding: 22, textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 12, color: DS.textSec, lineHeight: 1.6, marginBottom: 14 }}>
                  Sua solicitação para <strong>{confirmado.opcao.titulo}</strong> foi protocolada com origem na tela <em>{telaAtual || '—'}</em>. Você receberá retorno pelo e-mail institucional.
                </div>
                <div style={{ background: DS.bg, padding: 12, borderRadius: 8, fontSize: 11, color: DS.textMuted, fontFamily: 'monospace', textAlign: 'left', marginBottom: 14 }}>
                  evento: canal_comunicacao<br/>
                  opção: {confirmado.opcao.id}<br/>
                  tela: {telaAtual || '—'}<br/>
                  perfil: {perfil}<br/>
                  timestamp: {confirmado.ts}
                </div>
